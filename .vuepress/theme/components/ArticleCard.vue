<template>
    <router-link 
        class="article-card relative flex flex-col rounded-lg z-10 cursor-pointer no-underline hover:no-underline" style="min-height: 16rem;"
    ...
    >
        ...
        >
            <div v-if="featured" class="absolute inset-x-0 bottom-0 p-6 pt-20 text-white" style="background-image: linear-gradient(to top, rgba(74, 85, 104,.6) 40%, rgba(0,0,0,0) 100%)">
                <div 
        ...
        <main class="flex flex-1 flex-col bg-primary rounded-b-lg p-6 no-underline" v-if="! featured">
            <header>
                <div
                    class="uppercase no-underline hover:no-underline p-1 tracking-wider text-ternary text-xs font-semibold mb-2" 
                    v-text="article.frontmatter.tags[0]"
                ></div>
                <div 
                    class="font-sans text-lg border-0 leading-tight text-primary font-semibold mb-2" 
                    v-text="article.title"
                ></div>
            </header>
            <section
                class="font-sans text-secondary" 
